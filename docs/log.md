...
## 矛盾内容
- `index.md` 仍写 C1 从飞书 Wiki 同步；`log.md` 架构决策已改为 Bitable 表 A 是正文真源，Wiki 不参与同步。
- `index.md` 仍明文保留 Bitable 标识和访问链接；`log.md` 明确记录公开仓不得放此类标识。
- `STATUS.md` 显示 `feishu-wiki-sync`、`task-pulse`、`feishu-ws`、`nl-hash-guard` 异常；`log.md` 又宣称 MemoryOps 与版本治理已落地完成。
- `tasks/ledger.md` 标记 C1-C4 为“批 4 启用”；`STATUS.md` 显示其中多个任务已在运行并产生日志。
- `projects/nupai-crm/STATUS.md`、`projects/nupai-store/STATUS.md` 仍写“系统初始化中”，但顶层 `STATUS.md` 已报告生产服务状态。

## 孤立文件
- `blueprints/nupai-versioning-v2.1.md` 未进入 `index.md` 目录结构，仅在日志中被提及。
- `runbooks/20260319-openclaw-jobs-json-delivery-fix.md` 未被 OpenClaw 主索引、任务台账或故障复盘引用。
- `runbooks/20260427-openclaw-23-item-deploy-checklist.md` 是一次性部署清单，缺少归档入口。
- `runbooks/20260503-dispatcher-launchd-plist-repair.md` 未与 dispatcher 常规运维文档建立链接。
- 多个 CRM / Store / Dify 细分 runbook 缺少统一索引，查找依赖文件名猜测。

## 过期内容
- `index.md` 更新时间仍停留在早期批次，未反映后续 Bitable 正文真源和版本治理变更。
- `tasks/ledger.md` 仍是批 1 骨架状态，未同步当前任务健康情况。
- `projects/*/STATUS.md` 仍为初始化骨架，明显未被 C3 接管更新。
- `log.md` 中 `batch-2-plan` 仍保留“dry-run / 待验证 / 等 review”等计划态措辞，未标记已执行或废弃。
- 多个 runbook 状态仍为“草稿”，但内容已被实际生产使用，缺少 CURRENT / DEPRECATED 复审标记。
...
## 矛盾内容
- `index.md` 仍称 C1 从飞书 Wiki 同步；`log.md` 架构决策已改为 Bitable 表 A 是正文真源，Wiki 不参与同步。
- `index.md` 明文保留 Bitable 标识与访问链接；`log.md` 明确规定公开仓不得放此类敏感标识。
- `STATUS.md` 显示多个 MemoryOps 任务异常；`log.md` 又宣称 MemoryOps / 版本治理已全链路落地完成。
- `tasks/ledger.md` 写 C1-C4 为“批 4 启用”；`STATUS.md` 显示其中多个任务已在运行并产生日志。
- 项目级 `projects/*/STATUS.md` 仍为“系统初始化中”，但顶层 `STATUS.md` 已报告生产服务健康状态。

## 孤立文件
- `blueprints/nupai-versioning-v2.1.md` 未进入 `index.md` 正式目录结构，主要只在日志中被提及。
- `runbooks/20260319-openclaw-jobs-json-delivery-fix.md` 未被 OpenClaw 主索引、任务台账或故障复盘引用。
- `runbooks/20260427-openclaw-23-item-deploy-checklist.md` 是一次性部署清单，缺少归档入口。
- `runbooks/20260503-dispatcher-launchd-plist-repair.md` 未与 dispatcher 常规运维文档建立链接。
- 多个 CRM / Store / Dify 细分 runbook 缺少统一索引，查找依赖文件名猜测。
...
## 矛盾内容
- `index.md` 仍描述 C1 从飞书 Wiki 同步；`log.md` 架构决策已改为 Bitable 表 A 是正文真源，Wiki 不参与 C1 同步。
- `index.md` 明文保留 Bitable 标识与访问链接；`log.md` 明确规定公开仓不得放此类敏感标识。
- `STATUS.md` 显示多个 MemoryOps 任务异常；`log.md` 又宣称 MemoryOps / 版本治理已全链路落地完成。
- `tasks/ledger.md` 仍写 C1-C4 为“批 4 启用”；`STATUS.md` 显示其中多个任务已在运行并产生日志。
- 项目级 `projects/*/STATUS.md` 仍为“系统初始化中”，但顶层 `STATUS.md` 已报告生产服务健康状态。
...
## 矛盾内容
- `index.md` 仍描述 C1 从飞书 Wiki 同步；`log.md` 架构决策已改为 Bitable 表 A 是正文真源，Wiki 不参与 C1 同步。
- `index.md` 明文保留 Bitable 相关标识与访问链接；`log.md` 明确规定公开仓不得放此类敏感标识。
- `STATUS.md` 显示多个 MemoryOps 任务异常；`log.md` 又宣称 MemoryOps / 版本治理已全链路落地完成。
- `tasks/ledger.md` 仍写 C1-C4 为“批 4 启用”；`STATUS.md` 显示其中多个任务已在运行并产生日志。
- 项目级 `projects/*/STATUS.md` 仍为“系统初始化中”，但顶层 `STATUS.md` 已报告生产服务健康状态。
...
## 矛盾内容
- `index.md` 仍描述 C1 从飞书 Wiki 同步；`log.md` 的架构决策已改为 Bitable 表 A 是正文真源，Wiki 不参与同步。
- `index.md` 仍明文保留 Bitable 相关标识和访问链接；`log.md` 明确规定公开仓不得放此类敏感标识。
- `STATUS.md` 显示多个 MemoryOps 任务异常；`log.md` 又宣称 MemoryOps / 版本治理已全链路落地完成。
- `tasks/ledger.md` 写 C1-C4 仍处于后续批次启用；`STATUS.md` 显示其中多个任务已运行并产生日志。
- 项目级 `projects/*/STATUS.md` 仍为“系统初始化中”，但顶层 `STATUS.md` 已报告生产服务健康状态。
...
## 矛盾内容
- `index.md` 仍称 C1 从飞书 Wiki 同步；`log.md` 架构决策已改为 Bitable 表 A 是正文真源，Wiki 不参与同步。
- `index.md` 明文保留 Bitable 标识和访问链接；`log.md` 明确记录公开仓不得放此类敏感标识。
- `STATUS.md` 显示多个 MemoryOps 任务异常；`log.md` 又宣称 MemoryOps / 版本治理已全链路落地完成。
...
## 矛盾内容
- `index.md` 仍写 C1 从飞书 Wiki 同步；`log.md` 架构决策已改为 Bitable 表 A 是正文真源，Wiki 不参与同步。
- `index.md` 明文保留 Bitable 标识与访问链接（已脱敏为 `[REDACTED]`）；`log.md` 明确规定公开仓不得放此类敏感标识。
- `STATUS.md` 显示多个 MemoryOps 任务异常；`log.md` 又宣称 MemoryOps / 版本治理已全链路落地完成。
- `tasks/ledger.md` 写 C1-C4 仍为后续批次启用；`STATUS.md` 显示其中多个任务已运行并产生日志。
- 项目级 `projects/*/STATUS.md` 仍为“系统初始化中”，但顶层 `STATUS.md` 已报告生产服务健康状态。
